...
public class TrafficController {
    ...
    public ResponseEntity<Void> vehicleEntry(@RequestBody final VehicleRegistered request) {
        log.info("ENTRY detected in lane {} at {} of vehicle with license plate {}.",
                request.lane(), TIMESTAMP_FORMATTER.format(request.timestamp()), request.licensePlate());

        var state = new VehicleState(request.licensePlate(), request.timestamp());
    ...
    public ResponseEntity<Void> vehicleExit(@RequestBody final VehicleRegistered request) {
        return repository.getVehicleState(request.licensePlate())
    ...
    private VehicleState storeVehicleExit(final VehicleState existingState, final VehicleRegistered request) {
        log.info(" EXIT detected in lane {} at {} of vehicle with license plate {}.",
                request.lane(), TIMESTAMP_FORMATTER.format(request.timestamp()), request.licensePlate());
    ...
    private VehicleState handlePossibleSpeedingViolation(final VehicleState state) {
        var excessSpeed = this.speedingViolationCalculator.determineSpeedingViolationInKmh(
